...
| Variable | Default | Description |
|----------|---------|-------------|
| `EMAIL_PROVIDER` | | `resend` or `smtp`; leave empty to disable email sending |
...
When configured, the agent gains a `send_document_email` tool that can email estimates and invoices to clients with PDF attachments.
