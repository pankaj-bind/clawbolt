...
When configured, the agent gains a specialist `qb_query` tool that can run read-only queries against QuickBooks Online to look up invoices, estimates, customers, items, payments, and more.
